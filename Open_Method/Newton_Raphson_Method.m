function Newton_Raphson_Method()
    ...
    % =====================================================================
    % 4. THE SHOOTING METHOD MECHANICS (NEWTON-RAPHSON CALCULUS TANGENT)
    % =====================================================================
    current_guess = 0.0; 
    tolerance = 1e-9; max_iter = 50; converged = false; 
    dg = 1e-7; % Numerical perturbation step size parameter
    
    fprintf('\n--- NEWTON-RAPHSON METHOD ITERATION LOG (NON-LINEAR) ---\n');
    fprintf('%-10s %-20s %-20s\n', 'Iter', 'Current Guess', 'Residual Error');
    
    for iter = 1:max_iter
        % Run structural simulation at the baseline state parameter guess
        ...
        % Run a second simulation perturbed by a tiny step size change (dg)
        sol_perturbed = RK4(f_system, s_mesh, [0; 0; current_guess + dg]); 
        error_perturbed = sol_perturbed(3, end);
        
        % Compute the localized slope derivative numerically (dError / dGuess)
        derivative = (error_perturbed - error_current) / dg;
        
        % NEWTON LAW CORRECTION STEP: Update baseline guess using tangent slope intersect
        current_guess = current_guess - (error_current / derivative);
    end
    if ~converged, error('The tangent derivative vector path failed to find convergence.'); end
    
    print_and_plot_results(s_mesh, final_solution, L, 'Newton-Raphson Method (Non-Linear)');
    
end  % Closes the main Newton_Raphson_Method function block
